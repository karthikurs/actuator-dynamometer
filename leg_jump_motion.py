...
#
    ...
    while True:
        try:
            ...
            p1, v1, t1 = parse_reply(await c1.set_position(position=np.NaN, velocity=0.5, maximum_torque=max_torque/6, stop_position=d_pos1,query=True), g=6)
            p2, v2, t2 = parse_reply(await c2.set_position(position=np.NaN, velocity=0.5, maximum_torque=max_torque/6, stop_position=d_pos2,query=True), g=6)
            ...
            np.savetxt("leg_jump.csv", torques, delimiter=",")
            print('Torques saved')
            await c1.set_stop()
            await c2.set_stop()
